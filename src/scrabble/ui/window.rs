...
use relm::{connect, timeout, IsA, Relm, Update, Widget};
use std::cmp::max;

use crate::scrabble::board::Tile;
use crate::scrabble::util::{Direction, Move, Position, Letter};

use super::interaction::{Msg, ClickData};
use super::ui_game::UIGame;
...
impl Win {
    ...
    fn set(&mut self, p: Position, color: &str) {
        let mut at = match self.model.get_board()[p] {
            _ => Letter::Blank,
            Tile::Letter(l) => l
        };
        let mut letter_char = match at {
            Letter::Blank => '?',
            Letter::Letter(l) => l
        };
        let mut score = self.model.get_bag().score(at);
        let l = self.get(p.col as i32, p.row as i32);
        if self.model.get_board().blanks.contains(&p) {
            // blank
            letter_char = (letter_char as u32 + 127215).try_into().unwrap(); // make square character https://unicode.org/charts/nameslist/n_1F100.html
            score = 0;
        }
        let b = self.back_colors[&STATE[p.row][p.col]];
        self.lset(l, color, letter_char, score, &b);
    }
    ...
    fn setup_board(&mut self, first: bool) {
        let game_board = self.model.get_board();
        for row in 0..15 {
            for col in 0..15 {
    ...
    /// Re-Renders the rack UI Element
